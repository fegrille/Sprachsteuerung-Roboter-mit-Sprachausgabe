...
void espeak_callback(const std_msgs::String::ConstPtr& line) {
    ...
    espeak_Synth(line->data.c_str(), line->data.length()+1, 0, POS_CHARACTER, 0, 
        espeakCHARS_AUTO | espeakPHONEMES | espeakENDPAUSE, NULL, NULL);    //Bereitet die Stimme auf den Text vor
    espeak_Synchronize();       //Spricht und wartet bis es fertig ist
    //ROS_INFO("Speaking: \"%s\"", line->data.c_str());
}

int main( int argc, char** argv ) {
	espeak_Initialize(AUDIO_OUTPUT_PLAYBACK, 0, NULL, 0); // Initialisieren von espeak
...
void dyn_cfg_callback(espeak_ros::EspeakConfig &cfg, uint32_t level) {      // Konfigurieren der Stimme
	// lock mutex before calling espeak functions
	boost::mutex::scoped_lock u_lock(mtx);
...
std::string getVoiceName(int v) {
	...
	switch (v) {
		...
		case 7: voice.assign("brazil");
			break;
	    case 8: voice.assign("english-mb-en1");
	        break;
